...
require_once 'Modele/bordereaux.php';

class ControleurBordereaux extends Controleur {
  private $user;
  private $frs;
  private $site_rec;
  private $bordereaux;
  ...
  public function __construct() {
	$this->user       = new user();
	$this->bordereaux = new bordereaux();
	$this->frs        = new frs();
  ...
  public function index() {
    ...
    //filtrer selon site reception
		if($this->requete->existeParametre("Site_Rec")){
	        ...
	        $bordereaux    = $this->bordereaux->getBordereaux();
		    $nb_bordereaux = $bordereaux->rowCount();
			$bordereaux    = $bordereaux->fetchAll();
 ...
 //Recherche selon critere d'une colonne
    ...
    $this->genererVue(array('bordereaux'=>$bordereaux, 'frss'=>$frss,  'sites_rec'=>$sites_rec, 'login'=>$login, 'user'=>$user,   'user'=>$user, 'menuNum'=>7));
  }
	  
}//end ContreolBoredereau